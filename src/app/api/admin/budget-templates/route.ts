// src/app/api/admin/budget-templates/route.ts
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabase.from('budget_templates').insert(body).select().single()
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { data: oldData } = await supabase.from('budget_templates').select('*').eq('id', id).single()
    
    const { data, error } = await supabase.from('budget_templates').update(body).eq('id', id).select().single()
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
