// src/lib/storage/server-storage.ts
// Server-side only storage operations
import { createClient } from '@/lib/supabase/server'
import { GeneratedReport } from '@/lib/reports/types'

export class ServerStorage {
  /**
   * Save a generated PDF to storage (server-side only)
   */
  ...
  /**
   * Get the stored PDF URL for a report (server-side)
   */
  ...
  /**
   * Check if a report has a stored PDF (server-side)
   */
  ...
  /**
   * Delete a stored PDF (server-side)
   */
